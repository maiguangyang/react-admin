...
import { Button, Card, Col, Progress, Row, Space, Spin, Tooltip } from 'antd';
...
const ProjectPage: FC = () => {
  ...
  const columns: IColumnsDataType<ITabelColumnType>[] = [
    ...
    },
    {
      ...
      render: (text: any, row) => {
        return (
          <Space size="middle">
            <Button type='link' onClick={() => downLoadConfirm(row)}>在线编译</Button>
            <Link to={`${row.id}/edit`} className="primary">下载</Link>
          </Space>
        );
      },
    },
  ...
  const downLoadConfirm = (row: ITabelColumnType) => {
    modal.confirm({
      title: '温馨提示',
          ...
          return (
            <Col key={item.id} className="gutter-row" span={6} xxl={4} xl={6} lg={8} md={12} sm={24} xs={24}>
              ...
              <Card style={{ padding: '1px' }}
                ...
                actions={[
                  <CloudSyncOutlined key="cloudSync" style={{ fontSize: '20px' }} onClick={() => downLoadConfirm(item)} />,
