...
import { ExtractColumnIndex, GenerateVariable } from "~@/hooks/useTableList";
...
const ProjectPage: FC = () => {
  ...
  const [tableData, setTableData] = useState<
    IProjectResultType | null | undefined
  >(null);
  ...
  const [getList, { loading, data }] = useGraphql<
    IProjectResultType,
    QueryProjectsArgs
  >(model, fields, variables).List();
  ...
  // 使用一个定时器计算编译进度条
  const handleRowTimes = useCallback(
    (id: string) => {
      if (!tableData) return;
      setTimeout(() => {
        setTableData((oldValue) => {
                ...
                <Card
                  ...
                  actions={[
                    <Tooltip key="cloudSync" title="同步更改内容">
                      <CloudSyncOutlined
                        style={{ fontSize: "20px" }}
                        onClick={(e) => downLoadConfirm(e, item)}
                      />
                    </Tooltip>,
                    <Tooltip key="ci" title="在线编译打包">
                      <CiOutlined
                        style={{ fontSize: "20px" }}
                        onClick={(e) => handleOnCi(e, item)}
                      />
                    </Tooltip>,
